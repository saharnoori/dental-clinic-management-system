...
class PressClass:
    def __init__(self, root):
       ...
       #======================== combbox ====================
        ...
        #====================== photo ======================
        self.logo = Image.open(resource_path("C:/Users/Ideapad 3/OneDrive/Desktop/CLINIC_APP/images/ENDO.jpeg"))
        ...
        lbl_type = Label(self.root, text="الحالة", font=("tajwal",15), bg='lightyellow').place(x=940,y=215)
        lbl_price = Label(self.root, text="المبلغ ", font=("tajwal",15), bg='lightyellow').place(x=710,y=215)
        lbl_con = Label(self.root, text="القسط", font=("tajwal",15), bg='lightyellow').place(x=500,y=215)
        ...
        lbl_cont = Label(self.root, text="الكلي", font=("tajwal",15), bg='lightyellow').place(x=935,y=255)
        
        
        txt_cont = Entry(self.root,textvariable=self.var_cont, font=('tajwal',15), bg='lightyellow',justify=CENTER).place(x=770,y=255,width=150)
        ...
        self.press_table = ttk.Treeview(tree_frame,columns=("cont","con","price","type","address","contact","date","age","gender","name","pid"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        
        scrollx.pack(side=BOTTOM,fill=X)
        scrolly.pack(side=RIGHT,fill=Y)
        scrollx.config(command=self.press_table.xview)
        scrolly.config(command=self.press_table.yview)
        
        self.press_table.heading("cont",text="الكلي")
        self.press_table.heading("con",text="القسط")
        self.press_table.heading("price",text="المبلغ")
        self.press_table.heading("type",text="الحالة")
        self.press_table.heading("address",text="العنوان")
        self.press_table.heading("contact",text="الهاتف")
        self.press_table.heading("date",text="التاريخ")
        self.press_table.heading("age",text="العمر")
        self.press_table.heading("gender",text="الجنس")
        self.press_table.heading("name",text="الاسم")
        self.press_table.heading("pid",text="التسلسل")

        
        self.press_table["show"]="headings"
        
        self.press_table.column("pid" ,width=20,anchor=NE)
        self.press_table.column("name" ,width=90,anchor=NE)
        self.press_table.column("gender" ,width=30,anchor=NE)
        self.press_table.column("age" ,width=30,anchor=NE)
        self.press_table.column("date" ,width=50,anchor=NE)
        self.press_table.column("contact" ,width=50,anchor=NE)
        self.press_table.column("address" ,width=50,anchor=NE)
        self.press_table.column("type" ,width=50,anchor=NE)
        self.press_table.column("price" ,width=40,anchor=NE)
        self.press_table.column("con" ,width=20,anchor=NE)
        self.press_table.column("cont" ,width=50,anchor=NE)
        self.press_table.pack(fill=BOTH,expand=1)
        self.press_table.bind("<ButtonRelease-1>",self.get_data)
        self.show()
    ...
    def record_id(self):
        ...
        UPDATE press
                    SET pid = (SELECT new_id FROM cte WHERE cte.pid = press.pid)
        """ )
        con.commit()    
    ...
    def add(self):
        con = sqlite3.connect(resource_path('clinic.db'))
        cur = con.cursor()
        if (self.var_con.get()==""  or self.var_cont.get()=="" or self.var_type.get()=="" or self.var_address.get()=="" or self.var_contact.get()=="" or self.var_price.get()=="" or self.var_date.get()=="" or self.var_age.get()=="" or self.var_gender.get()=="select" or self.var_name.get()==""):
    ...
    def show(self):
        ...
        try:
            ...
            rows = cur.fetchall()
            self.press_table.delete(*self.press_table.get_children()) 
            for row in rows:
                self.press_table.insert('', END, values=row)
    ...
    def get_data(self,ev):
        f = self.press_table.focus()
        contant =(self.press_table.item(f))
        row = contant['values']
    ...
    def update(self):
        con = sqlite3.connect(resource_path('clinic.db'))
        cur = con.cursor()
        cur.execute("""Update press set cont=?, con=?, price=?, type=?,address=?,contact=?,date=?,age=?,gender=?,name=? where pid=?""",(
    ...
    def delete(self):
        ...
        op == True
        cur.execute("delete from press where pid=?", (self.var_id.get(),))
        con.commit()
        
        messagebox.showinfo('delete', "delete from press")
    ...
    def search(self):
        ...
        try:
            if self.var_search_comb.get()=='Select':
                ...
                query = f"SELECT * FROM press WHERE {column} LIKE ?"
